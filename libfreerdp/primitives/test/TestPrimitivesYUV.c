...
static BOOL TestPrimitiveYUVCombine(void)
{
	...
	fprintf(stderr, "Running YUVCombine on frame size %lux%lu\n",
	...
	for (x=0; x<3; x++)
	{
		...
		size_t halfSize = ((x>0)?halfStride*(aheight+1)/2:awidth*aheight);
...
static BOOL TestPrimitiveYUV(BOOL use444)
{
	...
	get_size(&roi.width, &roi.height);
	awidth = (roi.width+3) & ~0x03;
	aheight = (roi.height+3) & ~0x03;
