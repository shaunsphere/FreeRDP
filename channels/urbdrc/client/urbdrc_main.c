 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT urbdrc_exchange_capabilities(URBDRC_CHANNEL_CALLBACK* callback, wStream* pBuffer)
{
	UINT32 MessageId;
	UINT32 FunctionId;
	...
	case RIM_EXCHANGE_CAPABILITY_REQUEST:
		error = urbdrc_process_capability_request(callback, pBuffer, MessageId);
		break;
 ...
 * @return 0 on success, otherwise a Win32 error code
 */
static UINT urbdrc_process_channel_notification(URBDRC_CHANNEL_CALLBACK* callback, wStream* pBuffer)
{
	UINT32 MessageId;
	UINT32 FunctionId;
	UINT error = CHANNEL_RC_OK;
	...
	case CHANNEL_CREATED:
		error = urbdrc_process_channel_create(callback, pBuffer, MessageId);
		break;
	...
	case RIMCALL_RELEASE:
		...
		transfer_data->urbdrc = urbdrc;
		transfer_data->pBuffer = Stream_New(NULL, Stream_GetRemainingLength(pBuffer));
		...
		Stream_Copy(pBuffer, transfer_data->pBuffer, Stream_Capacity(transfer_data->pBuffer));
...
static UINT urbdrc_on_data_received(IWTSVirtualChannelCallback* pChannelCallback, wStream* data)
{
	...
	default:
		...
		transfer_data->pBuffer = Stream_New(NULL, Stream_GetRemainingLength(data));
		if (!transfer_data->pBuffer)
		{
			free(transfer_data);
			return ERROR_OUTOFMEMORY;
		...
		Stream_Copy(data, transfer_data->pBuffer, Stream_Capacity(transfer_data->pBuffer));
		Stream_SetPosition(transfer_data->pBuffer, 0);
...
static BOOL urbdrc_set_subsystem(URBDRC_PLUGIN* urbdrc, char* subsystem)
{
